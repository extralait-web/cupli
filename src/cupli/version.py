...
def version_info() -> str:
    ...
    struct_dir = Path(__file__).parents[1].resolve()
    most_recent_commit = git.git_revision(struct_dir) if git.is_git_repo(struct_dir) and git.have_git() else "unknown"

    info = {
        "cupli version": VERSION,
