...
if TYPE_CHECKING:
    ...
    from cupli.domain.models import AppModel, BaseAppModel, ExportModel, MountModel, SpaceModel
...
EXPORT_RESERVED: frozenset[str] = frozenset(("EXPORT_NAME", "EXPORT_PATH", "EXPORT_EXEC_PATH"))
"""Reserved variable names exported by ``exports[*]`` entries."""
...
class ResolvedSpace:
    ...
    Attributes:
        ...
        exports: resolved export entries (``path`` is the host destination;
            ``EXPORT_EXEC_PATH`` in ``vars`` is the resolved container source).
    ...
    exports: dict[str, ResolvedComponent] = field(default_factory=dict)
...
def _resolve_exports(
    ...
    *,
    ...
    """Resolve every declared export entry."""
    return {
        name: _resolve_export(
            name=name,
            export=export,
            outer=space_vars,
            strict=strict,
        ...
        for name, export in space.exports.items()
    }
...
def _resolve_export(
    *,
    ...
    """Resolve a single export's host ``path`` and container ``EXPORT_EXEC_PATH``."""
    path = Path(substitute(export.path, outer, strict=strict)).resolve()
    exec_path = substitute(export.exec_path, outer, strict=strict)
    auto = {
        "EXPORT_NAME": name,
        "EXPORT_PATH": str(path),
        "EXPORT_EXEC_PATH": exec_path,
    }
    scope = merge_scopes([outer, auto], strict=strict)
    _ = allow_shadow  # exports declare no user vars; nothing to shadow-check.
    return ResolvedComponent(name=name, path=path, vars=scope, container_vars={})


def _bridge_path_vars(space: SpaceModel, mounts: dict[str, ResolvedComponent]) -> dict[str, str]:
    """Build ``<MOUNT>_BRIDGE_PATH`` for mounts with an explicit ``host_bridge.link``.

    Auto-derived links depend on the resolved compose binds (docker) and are
    intentionally omitted here so variable resolution stays side-effect free.
    """
    out: dict[str, str] = {}
    for name, mount in space.mounts.items():
        if not mount.bridge_enabled:
            continue
        link = mount.bridge_spec.link
        if link is None:
            continue
        out[f"{_to_env_ident(name)}_BRIDGE_PATH"] = str(Path(substitute(link, mounts[name].vars)).resolve())
    return out


_PATH_VAR_SUFFIXES: tuple[str, ...] = ("_APP_PATH", "_BASE_PATH", "_MOUNT_PATH", "_EXPORT_PATH", "_BRIDGE_PATH")
...
def _container_subset(scope: dict[str, str], local_auto: dict[str, str]) -> dict[str, str]:
    ...
    excluded = (
        ...
        | MOUNT_RESERVED
        | EXPORT_RESERVED
        | frozenset(DEFAULT_ENV_ALLOWLIST)
        | frozenset(local_auto)
    )
